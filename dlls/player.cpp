		...
		if ( !FNullEnt( tr.pHit ) )
		{
			...
			if (pEntity->Classify() == CLASS_PLAYER )
			{
				...
				if ( g_pGameRules->PlayerRelationship( this, pEntity ) == GR_TEAMMATE )
				{
					...
					newSBarState[ SBAR_ID_TARGETARMOR ] = pEntity->pev->armorvalue; //No need to get it % based since 100 it's the fmax.
				}
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	{
#define clamp( val, fmin, fmax ) ( ((val) > (fmax)) ? (fmax) : ( ((val) < (fmin)) ? (fmin) : (val) ) )
